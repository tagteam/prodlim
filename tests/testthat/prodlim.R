...
test_that("prodlim",{
    ...
    ## graphics::plot(s1a,confint=FALSE,atrisk=FALSE,legend.x="bottomleft",legend.cex=0.8)
    ...
    ## graphics::plot(s3,confint=FALSE,atrisk=FALSE,legend.x="bottomleft",legend.cex=0.8,newdata=expand.grid(X1=levels(d$X1),X2=c(quantile(d$X2,0.05),median(d$X2))))
    ...
    ## graphics::plot(f1a,confint=FALSE,atrisk=FALSE,legend.x="bottomleft",legend.cex=0.8)
    ...
    ## graphics::plot(f3,confint=FALSE,atrisk=FALSE,legend.x="bottomleft",legend.cex=0.8,newdata=expand.grid(X1=levels(d$X1),X2=c(quantile(d$X2,0.05),median(d$X2))))
    data(pbc)
    prodlim.0 <- prodlim(Hist(time,status!=0)~1,data=pbc)
    survfit.0 <- survfit(Surv(time,status!=0)~1,data=pbc)
    ## graphics::plot(survfit.0)
    ## graphics::plot(prodlim.0,add=TRUE,col=2,lwd=3)
    ttt <- sort(unique(d$time)[d$event==1])
    ttt <- ttt[-length(ttt)]
    sum0.s <- summary(survfit.0,times=ttt)
    ## graphics::plot(survfit.0,lwd=6)
    ## graphics::plot(prodlim.0,add=TRUE,col=2)
    ...
    ## graphics::plot(s1,col=1,confint=FALSE,lwd=8)
    s2 <- prodlim(Hist(time,status>0)~1,data=pbc[sort(boot),])
    ## graphics::plot(s2,add=TRUE,col=2,confint=FALSE,lwd=3)
})
test_that("weights and delay",{
    ...
    ## graphics::plot(survey.1,lwd=8)
    prodlim.1 <- prodlim(Hist(time,status>0)~1,data=pbc,caseweights=pbc$randprob)
    ## graphics::plot(prodlim.1,add=TRUE,col=2,confint=FALSE)
    ...
    ## graphics::plot(survfit.delay,lwd=8)
    ## graphics::plot(prodlim.delay,lwd=4,col=2,add=TRUE,confint=FALSE)
    pbc0 <- pbc
    ...
    ## graphics::plot(survfit.delay.edema,conf.int=FALSE,col=1:3,lwd=8)
    ## graphics::plot(prodlim.delay.edema,add=TRUE,confint=FALSE,col=c("gray88","orange",5),lwd=4)
    data(abortion)
    ...
    graphics::plot(cif.ab.etm,lwd=8,col=3)
    graphics::plot(cif.ab.prodlim,add=TRUE,lwd=4,col=5,cause=3)
    data(abortion)
    ...
    cif.ab.prodlim <- prodlim(Hist(time=exit, event=cause,entry=entry) ~ 1,data=abortion)
    # cause 3
    ## graphics::plot(cif.ab.etm, ci.type = "bars", pos.ci = 24, col = c(1, 2), lty = 1,which.cif=3,lwd=8)
    ## graphics::plot(cif.ab.prodlim,add=TRUE,cause=3,confint=TRUE,col=2)
    # cause 2
    ## graphics::plot(cif.ab.etm, ci.type = "bars", pos.ci = 24, col = c(1, 2), lty = 1,which.cif=2,lwd=8)
    ## graphics::plot(cif.ab.prodlim,add=TRUE,cause=2,confint=TRUE,col=2)
    # cause 1
    ## graphics::plot(cif.ab.etm, ci.type = "bars", pos.ci = 24, col = c(1, 2), lty = 1,which.cif=1,lwd=8)
    ## graphics::plot(cif.ab.prodlim,add=TRUE,cause=1,confint=TRUE,col=2)
    data(abortion)
    ...
    ## graphics::plot(cif.ab.etm, ci.type = "bars", pos.ci = 24, col = c(1, 2), lty = 1, curvlab = c("Control", "Exposed"),lwd=8)
    ## graphics::plot(cif.ab.prodlim,add=TRUE,cause=3,confint=FALSE,col="yellow")
    ...
    ## graphics::plot(cif.test.etm, ci.type = "bars", pos.ci = 24, lwd=5)
    ## graphics::plot(cif.test.etm, ci.type = "bars", pos.ci = 24, lwd=5)
    ## graphics::plot(cif.test.prodlim,add=TRUE,cause=2,col=2,confint=TRUE,type="cuminc")
    ...
    ## graphics::plot(survfit.delayed.ill,lwd=5)
    ## graphics::plot(prodlim.delayed.ill,lwd=2,col=2,add=TRUE)
})
